...
uint8_t *LoadRef( arg_t *Arg )
{
    uint8_t *fa, *cur, ch;

    FILE *fp = fopen(Arg->ref, "r");
    if ( !fp ) fprintf(stderr, \
            "[Err::%s::%d] Failed to open %s!\n", __func__, __LINE__, Arg->ref);
    fa = cur = (uint8_t *)malloc(Arg->gsize * sizeof(uint8_t));
    if ( !fa ) 
        fprintf(stderr, \
...
static void BaseSort( pile_t *pile )
{
...
void Pileup( pile_t *P, tree_t *tree, arg_t *Arg )
{
    mut_t *snv; 
    int *clist; uint64_t cellnum = 1 << Arg->generation;
    ...
    for ( int b=0; b < Arg->gsize; b+=FRAGSIZE) {
        int start=b, end, index; uint64_t pos;
        end = b+FRAGSIZE > Arg->gsize ? Arg->gsize : b+FRAGSIZE;
        for ( int i=0; i < Arg->depth; i++ ) { /* random select the cell from pool */
            pos = RAND64() % cellnum; 
            index = LocIndex(clist, tree->typenum, pos);
            snv = tree->root[index].snv; DEALSNV(P, snv, start, end);
        }
    } free(clist);
    for ( int i=0; i < Arg->gsize; i++ )  BaseSort(&P[i]);
}

pile_t *PileInit( uint8_t *ref, arg_t *Arg )
{
    pile_t *P; uint8_t mtype[4] = {65, 67, 71, 84};

    P = (pile_t *)calloc(Arg->gsize, sizeof(pile_t));
    if ( !P ) {
        fprintf(stderr, \
                "[Err::%s::%d] Failed to allocate memory!\n", __func__, __LINE__);
        return NULL;
    }
    for ( int i=0; i < Arg->gsize; i++ ) {
        memcpy(P[i].base, mtype, 4*sizeof(uint8_t));
        P[i].num[0] = Arg->depth; P[i].base[0] = ref[i];
...
void SnvWrite( pile_t *P, arg_t *Arg )
{
    FILE *fp = fopen(Arg->snvfile, "w");
    float altfreq;

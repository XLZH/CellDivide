...
static void InsertAlt( mut_t *new, const mut_t *f, uint32_t pos, uint8_t type )
{
...
static mut_t *Mutator( const mut_t *f, arg_t *Arg )
{
    uint8_t type, mtype[4] = {65, 67, 71, 84};
    mut_t *snv; uint64_t pos = 0;

    pos = RAND64() % Arg->rate; type = mtype[rand() % 4];
    if ( pos <= Arg->gsize ) {
...
static void CreatCell( tree_t *tree, mut_t *new, int g )
{
...
void Divide( tree_t *tree, int g, arg_t *Arg )
{
...
void Destory( tree_t *tree )
{
    ...
    for ( int i=0; i < tree->typenum; i++ ) {
        ...
        snv->pos = NULL; snv->type = NULL; snv = NULL;
    }
